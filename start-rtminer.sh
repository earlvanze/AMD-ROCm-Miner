...
#nohup python /rootr/Claymore/stratum_proxy.py --local-host 1.2.3.4 --local-port 8008 --remote-host daggerhashimoto.usa.nicehash.com --remote-port 3353 --wallet-address 3NktvtucrCFRDdNi8aHiA9ozpYDnKZxDkJ.Rig0 &
nohup python /rootr/Claymore/stratum_proxy.py --local-host 1.2.3.4 --local-port 8008 --remote-host eth-us-west.flexpool.io --remote-port 4444 --wallet-address 0xbB6915E489C771E04A7f31feAD29A3db77083fd8.Rig0 &
...
echo 100 > echo  /proc/`ps -aux | grep teamredminer |egrep -v grep | awk '{print $2}'`/timerslack_ns
